...
public class AuctionData extends Model {
		...
		if (ad == null) {
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("INSERT INTO AuctionData (`buyout`, `count`, `curbid`, `ilevel`, `itemid`, `link`, `minbid`, `mininc`, `name`, `price`, `seed`, `seller`, `time`, `tleft`, `ulevel`, `updated`, `item_id`, `scantime`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				...
				while (matcher.find()) {
					...
					// `buyout`, `count`, `curbid`, `ilevel`, `itemid`, `link`,
					// `minbid`, `mininc`, `name`, `price`, `seed`, `seller`,
					// `time`, `tleft`, `ulevel`, `updated`, `item_id`,
					// `scantime`
	...
	public static void updatePrices() throws SQLException {
		...
		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.avgbuy = IFNULL(p.buyout,0),  i.avgbid = IFNULL(p.minbid,0),  i.avgbuyout = IFNULL(p.buyout,0)");
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 12 HOUR AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.buyout12H = IFNULL(p.buyout,0),  i.minbid12H = IFNULL(p.minbid,0)");
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 3 DAY AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.buyout3Day = IFNULL(p.buyout,0),  i.minbid3Day = IFNULL(p.minbid,0)");
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 10 DAY AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.buyout10Day = IFNULL(p.buyout,0),  i.minbid10Day = IFNULL(p.minbid,0)");
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 30 DAY AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.minbid30Day = IFNULL(p.buyout,0),  i.minbid30Day = IFNULL(p.minbid,0)");
		ps.executeUpdate();
	...
	public static void updatePrices(Long itemId) throws SQLException {
		Connection con = DB.getConnection();
		PreparedStatement ps;

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.avgbuy = IFNULL(p.buyout,0),  i.avgbid = IFNULL(p.minbid,0),  i.avgbuyout = IFNULL(p.buyout,0) where i.itemId = ?");
		ps.setLong(1, itemId);
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 12 HOUR AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.buyout12H = IFNULL(p.buyout,0),  i.minbid12H = IFNULL(p.minbid,0) where i.itemId = ?");
		ps.setLong(1, itemId);
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 3 DAY AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.buyout3Day = IFNULL(p.buyout,0),  i.minbid3Day = IFNULL(p.minbid,0) where i.itemId = ?");
		ps.setLong(1, itemId);
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 10 DAY AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.buyout10Day = IFNULL(p.buyout,0),  i.minbid10Day = IFNULL(p.minbid,0) where i.itemId = ?");
		ps.setLong(1, itemId);
		ps.executeUpdate();

		ps = con.prepareStatement("UPDATE Item AS i LEFT JOIN (select round(avg(ad.buyout / ad.count)) as buyout, round(avg(ad.minbid / ad.count)) as minbid, ad.itemid as itemId from AuctionData ad where ad.time BETWEEN SYSDATE() - INTERVAL 30 DAY AND SYSDATE() group by ad.itemid, ad.name) AS p ON i.itemId = p.itemid SET i.minbid30Day = IFNULL(p.buyout,0),  i.minbid30Day = IFNULL(p.minbid,0) where i.itemId = ?");
		ps.setLong(1, itemId);
		ps.executeUpdate();
