...
public class WoWAPI {
	...
	private void setLastLogs() {
		...
		try {
			if (checkUpdate(new Date(), log.lastUpdate, WoWAPI.QUATERDAYUPDATE)) {
				update = true;
			}
	...
	private void fetchLogs() {
		...
		for (JsonElement jsonElement : ja) {
			...
			if (log == null) {
				...
				ArrayList<WoWCharacter> participants = new ArrayList<WoWCharacter>();
				...
				for (JsonElement jaParticipant : jaParticipants) {
					...
					WoWCharacter participant = WoWCharacter.find("name = ? and realm.name = ?", name, Play.configuration.getProperty("wowapi.realmName")).first();
					if (participant == null) {
	...
	public static WoWCharacter fetchCharacter(String vRealm, String vName) {
		WoWCharacter m = WoWCharacter.find("name = ? and realm.name = ?", vName, vRealm).first();
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), m.lastUpdate, WoWAPI.QUATERDAYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	public static WoWCharacter fetchCharacter(Boolean update, String vRealm, String vName) {

		JsonObject character = fetchFromArmory(WoWAPI.CHARURL, "guild,stats,talents,items,professions,appearance,progression,pvp", vRealm, vName).getAsJsonObject();
		...
		WoWCharacter gm = WoWCharacter.find("name = ? and realm.name = ?", gmname, gmrealm.name).first();
		...
		// Long classId =
		// jsonElement.getAsJsonObject().get("class").getAsLong();
		// String name =
		// jsonElement.getAsJsonObject().get("name").getAsString();
		//
		// ItemClass ic = new ItemClass(classId, name);
		// ic.save();
		// }
	...
	public static void setRealms() {
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), realm.lastUpdate, WoWAPI.QUATERDAYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	private static void fetchRealms() {
		JsonArray realms = fetchFromArmory(WoWAPI.REALMSURL).getAsJsonObject().get("realms").getAsJsonArray();
	...
	public static void setMainGuild() {
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), mGuild.lastUpdate, WoWAPI.QUATERDAYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	private static Guild fetchGuild(Boolean isMainGuild, String vname, Realm vrealm) {
		JsonObject mGuild;
		if (isMainGuild) {
		...
		Long area_rank = 0L;
		Long realm_rank = 0L;
		JsonElement eRankings =  fetchFromWoWProgress(WoWAPI.WOWPROGRESSURL + g.realm.region.toLowerCase() + "/" + replaceSpecialChars(g.realm.name.toLowerCase()) + "/" + encodeChars(g.name) + "/json_rank");
	...
	private static JsonObject getGuildJson(String vname, Realm vrealm) {
		JsonObject mGuild = fetchFromArmory(WoWAPI.GUILDURL, "achievements,members", vrealm.name, vname).getAsJsonObject();
		return mGuild;
	}

	public static JsonObject getMainGuildJson() {
		JsonObject mGuild = fetchFromArmory(WoWAPI.GUILDURL, "achievements,members", Play.configuration.getProperty("wowapi.realmName"), Play.configuration.getProperty("wowapi.guildName"))
				.getAsJsonObject();
		return mGuild;
	}
	...
	public static void setItemClasses() {
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), iClass.lastUpdate, WoWAPI.WEEKLYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	private static void fetchItemClasses() {
		JsonArray classes = fetchFromArmory(WoWAPI.ITEMCLASSESURL).getAsJsonObject().get("classes").getAsJsonArray();
	...
	public static void setGuildPerks() {
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), gPerk.lastUpdate, WoWAPI.WEEKLYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	private static void fetchGuildPerks() {
		JsonArray perks = fetchFromArmory(WoWAPI.GUILDPERKSURL).getAsJsonObject().get("perks").getAsJsonArray();
	...
	public static void setCharacterRaces() {
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), cRace.lastUpdate, WoWAPI.WEEKLYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	private static void fetchCharacterRaces() {
		JsonArray races = fetchFromArmory(WoWAPI.CHARACTERRACESURL).getAsJsonObject().get("races").getAsJsonArray();
	...
	public static void setCharacterClasses() {
		...
		Boolean update = false;
		try {
			if (checkUpdate(new Date(), cClass.lastUpdate, WoWAPI.WEEKLYUPDATE)) {
				update = true;
			}
		} catch (NullPointerException e) {
			update = true;
	...
	private static void fetchCharacterClasses() {
		JsonArray classes = fetchFromArmory(WoWAPI.CHARACTERCLASSESURL).getAsJsonObject().get("classes").getAsJsonArray();
	...
	private static JsonElement fetchFromArmory(String moduleURL, String fields, String realm, String guild, String character, String locale, String region) {
		...
		JsonElement armoryanswer = WS.url(battleNetApiURL).get().getJson();
		return armoryanswer;
	}
