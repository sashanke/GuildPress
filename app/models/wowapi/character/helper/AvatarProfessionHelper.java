...
public class AvatarProfessionHelper {
	...
	public static List<AvatarProfessionHelper> getList(AvatarProfession avatarProfession) {
		...
		try {
			PreparedStatement ps = DB.getConnection().prepareStatement("select ap.avatar_id, re.id as recipe_id from AvatarRecipe ar join AvatarProfession ap on (ar.avatarProfession_id = ap.id) join Recipe re on (ar.recipe_id = re.id and re.name is not null) join Item im on (re.item_id = im.id and im.name is not null) where ap.profId = ? order by re.profLevel desc, im.level desc");
