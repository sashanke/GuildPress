...
public class GuildMember extends Model {
	...
	public static GuildMember findByNameAndRealm(String name, Realm realm) {
		...
		// DB.getConnection().prepareStatement("select id from GuildMember where BINARY name = ? and BINARY realm_id = ?");
		// ps.setString(1, name);
		// ps.setLong(2, realm.id);
		// ResultSet rs = ps.executeQuery();
		// if (rs.first()) {
		// Logger.info("[GuildMember][findByNameAndRealm] GuildMember " + name +
		// " found");
		...
		// }
		// } catch (SQLException e) {
		// Logger.warn("[GuildMember][FindByNameAndRealm] " +
